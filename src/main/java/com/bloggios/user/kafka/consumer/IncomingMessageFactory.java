...
import static com.bloggios.user.constants.BeanConstants.MANAGEMENT_CONSUMER_FACTORY;
...
@Component(INCOMING_MESSAGE_FACTORY_COMPONENT)
@DependsOn(MANAGEMENT_CONSUMER_FACTORY)
public class IncomingMessageFactory extends ConcurrentKafkaListenerContainerFactory<String, IncomingMessage> {
    ...
    public IncomingMessageFactory(
            @Qualifier(MANAGEMENT_CONSUMER_FACTORY) ConsumerFactory<String, IncomingMessage> consumerFactory
    ) {
    ...
    public void configure() {
        ...
        getContainerProperties().setAckMode(ContainerProperties.AckMode.MANUAL);
    }
}
